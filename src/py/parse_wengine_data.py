...
def load_initial_weapon(weapons, translation_json):
    with open(f"C:/facul/ZenlessData/FileCfg/WeaponTemplateTb.json", "r", encoding="utf-8") as json_file:
...
def load_weapon_refinement(weapons, translation_json):
    with open(f"C:/facul/ZenlessData/FileCfg/WeaponTalentTemplateTb.json", "r", encoding="utf-8") as json_file:
        json_to_parse = json.load(json_file)
...
for x in weapon_dict.values():
    with open(f"../data/wengines/{x['Id']}.json", "w", encoding="utf-8") as json_file_to_write:
